...
{
	int num = static_cast<uint8_t>(selectedBlock);
	...
	if (num >= static_cast<uint8_t>(BlockId::blockCount))
		num = 1;
	else if (num < 1)
		num = static_cast<uint8_t>(BlockId::blockCount) - 1;
...
void Player::SetCamera(glm::dvec3 newPos)
{
...
{
	//TODO menos acelera��o quando se movendo no ar
	m_isGrounded = false;
...
//TODO ainda est� um pouco bugado
bool Player::CollidingWithBlock(const glm::ivec3 blockPos) const
{
	...
	if (Floor(m_position + glm::dvec3{ -m_dimensions.sides, -m_dimensions.bottom,  m_dimensions.sides }) == blockPos)
		return true;
	if (Floor(m_position + glm::dvec3{ -m_dimensions.sides,  m_dimensions.top,	  -m_dimensions.sides }) == blockPos)
		return true;
	if (Floor(m_position + glm::dvec3{ -m_dimensions.sides,  m_dimensions.top,	   m_dimensions.sides }) == blockPos)
		return true;
	if (Floor(m_position + glm::dvec3{  m_dimensions.sides, -m_dimensions.bottom, -m_dimensions.sides }) == blockPos)
	...
	if (Floor(m_position + glm::dvec3{  m_dimensions.sides, -(m_dimensions.top + m_dimensions.bottom), -m_dimensions.sides }) == blockPos)
		return true;
	if (Floor(m_position + glm::dvec3{  m_dimensions.sides, -(m_dimensions.top + m_dimensions.bottom),  m_dimensions.sides }) == blockPos)
		return true;
	if (Floor(m_position + glm::dvec3{ -m_dimensions.sides, -(m_dimensions.top + m_dimensions.bottom), -m_dimensions.sides }) == blockPos)
...
{
	int iter = 0;
	for(int x = floor(m_position.x - m_dimensions.sides); x < m_position.x + m_dimensions.sides; x++)
	for(int y = floor(m_position.y - m_dimensions.bottom); y < m_position.y + 0.2		   ; y++)
	for(int z = floor(m_position.z - m_dimensions.sides); z < m_position.z + m_dimensions.sides; z++)
	{
		iter++;
		...
		{
			if (vel.y > 0) {
				m_position.y = y - m_dimensions.top - tolerance;
				m_velocity.y = 0;
			}
			else if (vel.y < 0) {
				m_isGrounded = true;
				m_position.y = y + m_dimensions.bottom + 1 + tolerance;
				m_velocity.y = 0;
			}

			if (vel.x > 0) {
				m_position.x = x - m_dimensions.sides - tolerance;
				m_velocity.x = 0;
			}
			else if (vel.x < 0) {
				m_position.x = x + m_dimensions.sides + 1 + tolerance;
				m_velocity.x = 0;
			}

			if (vel.z > 0) {
				m_position.z = z - m_dimensions.sides - tolerance;
				m_velocity.z = 0;
			}
			else if (vel.z < 0) {
				m_position.z = z + m_dimensions.sides + 1 + tolerance;
				m_velocity.z = 0;
			}
		}
	}
